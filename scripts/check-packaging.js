...
// Distribution manifests refer to already published binaries/checksums. During
// release preparation they can lag the app version until new assets exist.
const scoop = JSON.parse(fs.readFileSync(path.join(root, 'packaging', 'scoop', 'protimer.json'), 'utf8'));
const version = scoop.version;
if (!/^\d+\.\d+\.\d+$/.test(version)) throw new Error('Packaging check failed: invalid distribution version');
...
if (!nuspec.includes(`<version>${version}</version>`)) fail('Chocolatey version does not match the published distribution version');
...
if (!install.includes('Install-ChocolateyShortcut')) fail('Chocolatey shortcut creation is missing');
if (!fs.existsSync(skipAutoUninstall)) fail('Chocolatey automatic uninstaller must be disabled');
if (!fs.existsSync(guiMarker)) fail('Chocolatey GUI shim marker is missing');
if (pkg.build?.nsis?.guid !== windowsAppGuid) fail('NSIS application GUID changed');
if (!uninstall.includes('[StringComparer]::OrdinalIgnoreCase.Equals($target, $appPath)')) {
  fail('Chocolatey uninstall must only remove package-owned shortcuts');
}
